...
class EnergyStar80(object):
    ...
    def __init__(self, sysinfo):
        ...
        debug("=== Energy Star 8.0 Draft 2 ===")

    # Requirements for Desktop, Integrated Desktop, and Notebook Computers
    def equation_one(self):
        """Equation 1: TEC Calculation (E_TEC) for
                       Desktop, Integrated Desktop, Thin Client and Notebook Computers"""
        ...
        if self.sysinfo.product_type == 4:
            ...
            (t_off, t_sleep, t_long_idle, t_short_idle) = (0.15, 0.45, 0.1, 0.3)
    ...
    def equation_two(self, fb_bw, mobile_workstation=False):
        ...
        msg = "TEC_BASE = %s, TEC_MEMORY = %s, TEC_GRAPHICS = %s, TEC_SWITCHABLE = %s, " + \
              "TEC_10GLAN = %s, TEC_STORAGE = %s, TEC_INT_DISPLAY = %s, TEC_MOBILEWORKSTATION = %s"

        debug(msg % (tec_base, tec_memory, tec_graphics, tec_switchable, tec_10glan, tec_storage,
                     tec_int_display, tec_mobile_workstation))

        return tec_base + tec_memory + tec_graphics + tec_storage + tec_int_display + \
               tec_switchable + tec_10glan + tec_mobile_workstation
